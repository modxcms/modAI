modAIAdmin.combo.ExtendedBoolean = function (config) {
  ...
  var data = [
    [_('modai.admin.global.any'), null, _('modai.admin.global.any')],
    [_('yes'), config.useInt ? 1 : true, _('yes')],
    [_('no'), config.useInt ? 0 : false, _('no')],
  ];

  if (config.dataLabel) {
    data = [
      [config.dataLabel + ': ' + _('modai.admin.global.any'), null, _('modai.admin.global.any')],
      [config.dataLabel + ': ' + _('yes'), config.useInt ? 1 : true, _('yes')],
      [config.dataLabel + ': ' + _('no'), config.useInt ? 0 : false, _('no')],
    ];
  }
...
modAIAdmin.combo.AgentType = function (config) {
  const data = [
    ['text', _('modai.admin.agent.type.text')],
    ['image', _('modai.admin.agent.type.image')],
  ];
  ...
  modAIAdmin.combo.AgentType.superclass.constructor.call(this, config);
};
Ext.extend(modAIAdmin.combo.AgentType, MODx.combo.ComboBox);
Ext.reg('modai-combo-agent_type', modAIAdmin.combo.AgentType);
...
modAIAdmin.combo.PromptLibraryCategory = function (config) {
  ...
  Ext.applyIf(config, {
    ...
    tpl: new Ext.XTemplate(
      '<tpl for="."><div class="x-combo-list-item">{name:htmlEncode} ({type})</div></tpl>',
    ),
  });
